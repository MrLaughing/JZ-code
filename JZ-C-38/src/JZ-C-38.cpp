...
int GetFirstK(int* data, int length, int k, int start, int end) {
	...
	if (middleData == k) {
		if ((middleIndex > 0 && data[middleIndex - 1] != k) || middleIndex == 0)// data[middleIndex - 1] != k 以此判断是不是第一个k
			return middleIndex;
		else
			end = middleIndex - 1;
...
int GetLastK(int* data, int length, int k, int start, int end) {
	...
	if (middleData == k) {
		if ((middleIndex < length - 1 && data[middleIndex + 1] != k)//data[middleIndex + 1] != k 以此判断是不是最后一个k
